...
export default function VehicleEditPage() {
  ...
  const handleFileChange = (e) => {
    ...
    if (files && files[0]) {
      ...
      alert(`[${name === 'carProfile' ? '차량 사진' : '차량 등록증'}] 첨부되었습니다: ${files[0].name}`);
    }
  };
  ...
  const handleSaveVehicle = async () => {
    ...
    try {
      ...
      alert(response.data.message || (isEditMode ? '차량 정보 수정이 완료되었습니다.' : '차량 등록이 완료되었습니다.'));
      navigate('/settings'); 
  ...
  return (
    <div className='vehicle-edit'>
      ...
      <main className='vehicle-edit__main'>
        <h1 className='vehicle-edit__title'>
          {isEditMode ? '내 차량 수정' : '내 차량 추가'}
        </h1>
        ...
        <section className='vehicle-edit__section'>
          <div className='vehicle-edit__upload-row'>
            ...
            <input 
              type="file" name="carProfile" accept="image/*" 
              ref={carProfileRef} style={{ display: 'none' }} 
          ...
          <div className='vehicle-edit__regist-section'>
            <div className='vehicle-edit__upload-row vehicle-edit__regist-row'>
              ...
              <input 
                type="file" name="registration" accept=".pdf, image/*" 
                ref={registrationRef} style={{ display: 'none' }} 
                onChange={handleFileChange}
              />
